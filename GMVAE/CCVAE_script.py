...
def sample_cc_ordered_reparam(lam):
    ...
    # ---- NEW: count failures
    # Will be 1 if a row fails, 0 if success
    failed_rows = torch.zeros(B, dtype=torch.bool, device=lam.device)
    ...
    if active_mask.any():
        failed_rows[active_mask] = True
        num_failed = active_mask.sum().item()
        print(f"[WARN][CC-Sampler] {num_failed}/{B} samples failed to accept within {max_attempts} attempts.")
...
def train_loop(model: 'CCVAE', vi: VariationalInference, optimizer, train_loader, device):
    ...
    print(f"TRAIN: Negative ELBO: {mean_loss:.4f} | Recon Loss: {mean_recon:.4f} | KL: {mean_kl:.4f}")
...
def test_loop(model: 'CCVAE', vi: VariationalInference, test_loader, device):
    ...
    print(f"TEST: Negative ELBO: {mean_loss:.4f} | Recon Loss: {mean_recon:.4f} | KL: {mean_kl:.4f}")
    return mean_loss, mean_recon, mean_kl
...
if __name__ == "__main__":
    ...
    trainset = filter_mnist(trainset, keep=[1,2,3,4,5,6])
    testset = filter_mnist(testset, keep=[1,2,3,4,5,6])
    ...
    vi = VariationalInference(
        zero_beta_epochs = 0,
        base_beta=1.0, 
        warmup_epochs=0, # Matches setting from train_from_scratch
        max_beta=100000.0, 
        C_free=20.0        # Matches setting from train_from_scratch (Free-Bits)
    ).to(device)
    ...
    if os.path.exists(f"saves/CCVAE/CCVAE_model_e_{NUM_EPOCHS}_ld_{latent_dim}{'_' + test_name if test_name != None else ''}.pth"):
        print("Loading trained model...")
        model.load_state_dict(torch.load(f"saves/CCVAE/CCVAE_model_e_{NUM_EPOCHS}_ld_{latent_dim}{'_' + test_name if test_name != None else ''}.pth", weights_only=True, map_location=device))
    else:
        model = train_from_scratch(model, vi, train_loader, test_loader, device)
        torch.save(model.state_dict(), f"saves/CCVAE/CCVAE_model_e_{NUM_EPOCHS}_ld_{latent_dim}{'_' + test_name if test_name != None else ''}.pth")
