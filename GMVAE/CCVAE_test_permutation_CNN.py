...
# ======================================================================
# 2. CNN MODEL DEFINITIONS (Replacing MLP)
# ======================================================================
...
class CNNDecoder(nn.Module):
    def __init__(self, latent_dim, hidden_channels, output_channels=1):
        super().__init__()
        ...
        self.net = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.ConvTranspose2d(32, output_channels, kernel_size=4, stride=2, padding=1), # -> 1 x 28 x 28
            # No Sigmoid here, handled by loss (BCEWithLogits) or reconstruction
        )
...
class CCVAE(nn.Module):
    def __init__(self, input_dim, enc_hidden_dims, dec_hidden_dims, latent_dim):
        super().__init__()
        # Note: enc_hidden_dims is now treated as channel width info or unused
        self.encoder = CNNEncoder(enc_hidden_dims, latent_dim)
        self.decoder = CNNDecoder(latent_dim, dec_hidden_dims)
...
class VariationalInference(nn.Module):
    ...
    def forward(self, x: Tensor, logits_dec: Tensor, lam: Tensor, z: Tensor, epoch: int):
        ...
        # FIX: Flatten inputs so sum(dim=1) works correctly for both MLP (2D) and CNN (4D)
        x_flat = x.view(B, -1)
        logits_flat = logits_dec.view(B, -1)
        
        bce = F.binary_cross_entropy_with_logits(logits_flat, x_flat, reduction='none')
        ...
        log_q = cc_log_prob_torch(z, eta_q) 
        log_p = cc_log_prob_torch(z, eta_p) 
...
def train_from_scratch(model: 'CCVAE',vi:VariationalInference, train_loader, test_loader, device):
    # Added weight decay to help stabilize distribution
    ...
    for epoch in range(0, NUM_EPOCHS+1):
        ...
        if epoch % 5 == 0:
            ...
            with torch.no_grad():
                ...
                # CNN output is already 4D [B, 1, 28, 28]
                recon_imgs = torch.sigmoid(logits_dec) 
                
                # Compare batch (which is 4D) with recon (4D)
                comparison = torch.cat([batch, recon_imgs])
...
if __name__ == "__main__":
    ...
    print(f"Using learning rate: {learning_rate}, hidden dim (channels): {hidden_dim}")

    # UPDATED TRANSFORM: No Flatten!
    transform = T.Compose([
        T.ToTensor(), 
    ])
    ...
    # Input dim is implicitly handled by CNN (1 channel)
    
    print(f"Training CNN-CCVAE with latent dim: {latent_dim}")
    print(f"used numbers: {nums}")
    print(f"epochs : {NUM_EPOCHS}")
    
    model = CCVAE(input_dim=None, # Unused for CNN
                  enc_hidden_dims=hidden_dim,
                  dec_hidden_dims=hidden_dim,
                  latent_dim=latent_dim).to(device)

    # LOWER BETA for better reconstruction of difficult digits
    vi = VariationalInference(
            zero_beta_epochs = 20,
            base_beta=0.3, 
            warmup_epochs=100, # Slower warmup
            max_beta=0.1,      # Lower cap
        ).to(device)
    
    test_name = f"cnn_test_lr_{learning_rate}_hd_{hidden_dim}"  
    skip_load = False 
    ...
    if not skip_load and os.path.exists(f"{path}/CCVAE_model_e_{NUM_EPOCHS}_ld_{latent_dim}.pth"): 
        ...
        # Plotting requires a flat vector for make_new_vae_plots usually, so we grab one and flatten just for that function if needed
        # But make_new_vae_plots likely just plots curves.
        # Passing a sample reconstruction:
        sample_in = next(iter(test_loader))[0][:1].to(device)
        sample_out = model(sample_in)[0]
        
        make_new_vae_plots(model, loss_data, recon_data, kl_data, sample_out, f'{performance_path}/performance_e_{NUM_EPOCHS}_ld_{latent_dim}.png') 
...
def generate_plots(model: CCVAE, test_loader, single_graph=None):
        ...
        # Note: plot_latent_dim_wise_reconstruct might need updates for CNN shapes if it manually feeds vectors
        # Usually it iterates dims and runs decoder. Our decoder expects [B, Latent], so it should be fine.
        if single_graph == None or single_graph == 'OH':
            fig, ax = plt.subplots()
            # This function might need inspection if it assumes MLP output shapes, 
            # but usually plotting libs handle the output image reshape.
